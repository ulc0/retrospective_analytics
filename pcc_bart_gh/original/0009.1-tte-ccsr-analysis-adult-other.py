...
# Get twj8_medrec_ccsr_wide_cohort_final
...
# Adjust Condition
# - Adult status true, 
# - then if chronic_disease_score >= 3, then 3
# - else chronic_disease_score = pmca_score
if ADULT_STATUS:
...
# Filter
# - adult = ADULT_STATUS and inpatient_status = INPATIENT_STATUS
# Create age group binned
# Create binary sex 1,0
# Create chronic_disease_score
# Create prior_hosp binary 1,0
# Create region_code factor 0,1,2,3
# select cols 'index_date', 'covid', 'inpatient_status', 'age', 'adult', 'gender', 'prov_region', 'cci_score', 'pmca_score', 'n_prior_hosp'
# Create e_condition as if condition 31-180 then 1, if -6-30 then 0, else null then 0
# Create d_condition as if condition 31-180 then condition - 31, if -6-30 then 149, if null then 149
cohort_df = (
...
# Run
# Create empy stats and failed_ccsrs lists
# Loop through conditions_list, get i, event (index, ccsr event)
# From cohort_df
# Select cat_feats, e_event as event and d_event as duration
# Filter
# - event Not Null
# Groupby cat_feats, duration, event
# create count as count() rows and create pandas
# Cox Model
# fit the event_pdf with
# - duration, event, weights_col = count, fourmula = covid + age_group + region_code + chronic_disease_score + sex + prior_hosp
# get summary stats
