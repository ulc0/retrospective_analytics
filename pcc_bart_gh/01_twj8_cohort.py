...
# MAGIC %sql
# MAGIC -- create schema if not exists bart_pcc
...
paticd=spark.read.table("cdh_premier.paticd_diag")
...
payor = spark.table("cdh_premier.payor")
...
# filter encounters included in the date_range_cond (2020-03 -> 2021-11)
def get_inclusion_period_rec(patdemo, DATE_RANGE_COND):
    out = (
        patdemo
        .select("pat_key", "medrec_key", "adm_mon", 
                F.substring(F.col("adm_mon"), 1,4).alias("year"))
        .withColumn(
            "adm_date",
            F.to_date(
                F.concat(
                    F.substring(F.col("adm_mon"), 1,4),
                    F.lit("-"),
                    F.substring(F.col("adm_mon"), 6,2)
                ),
                "yyyy-MM"
            )
        )
...
def get_covid_incl_icd(inc_dtst, paticd):
    ...
    out = (
        icd
        .join(inc_dtst, on = "pat_key", how = "inner")
        .withColumn("covid",
                    cov_cond
        )
...
def get_primary_date(inc_enc2):
    out = (
        ...
        .select(
            ...
            .alias('covid_patient')
            , F.row_number()
            .over(W.Window.partitionBy('medrec_key').orderBy(F.desc('covid'),'adm_date'))
            .alias('date_rn')
            , F.row_number()
            .over(W.Window.partitionBy('medrec_key').orderBy('adm_date').orderBy(F.rand(42)))
            .alias('rand_rn')
        )
        .filter(
            (
...
# write intermediate
enc_primary.write.mode("overwrite").format("parquet").saveAsTable("bart_pcc.01_enc_primary")

# COMMAND ----------

# read intermediate
enc_primary1 = spark.table("bart_pcc.01_enc_primary")
...
enc_excl1.write.mode("overwrite").format("parquet").saveAsTable("bart_pcc.02_demographics")
enc_excl2 = spark.table("bart_pcc.02_demographics")
...
#     print(samp1.count())
    samp1.write.mode("overwrite").format("parquet").saveAsTable("bart_pcc.sample1")
    enc_exl = spark.read.table("bart_pcc.sample1")
else:
    enc_exl = enc_excl2
...
# Validation that adm_date is distrubited mostly uniform, with a slight taper off in the first months of the covid inclusion period
...
exl4.write.mode("overwrite").format("delta").saveAsTable("bart_pcc.03_enrollment_exclusion2")
exl4 = spark.table("bart_pcc.03_enrollment_exclusion2")
...
coh.write.mode("overwrite").format("delta").saveAsTable("bart_pcc.04_exclusion_noncov_cov2")
coh = spark.table("bart_pcc.04_exclusion_noncov_cov2")
...
# coh = spark.table("bart_pcc.04_exclusion_noncov_cov")
...
coh_tte.write.mode("overwrite").format("delta").saveAsTable("bart_pcc.05_ccsrs_tte2")
...
# #  .format("parquet")
#  .format("delta")
#  .saveAsTable("bart_pcc.01_enc_primary")
# )
# # read in file
# enc_primary1 = spark.table("bart_pcc.01_enc_primary")
...
# #      .format("parquet")
#     .format("delta")
#     .saveAsTable("bart_pcc.02_demographics")
# )
# enc_excl2 = spark.table("bart_pcc.02_demographics")
...
# #  .format("parquet")
#  .format("delta")
#  .saveAsTable("bart_pcc.03_ccsrs_tte")
# )
