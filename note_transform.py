...
dbutils.widgets.text("NOTE_TABLE", defaultValue="edav_dev_cdh.cdh_mimic_ra.note") #,patientnoteproblem,patientnoteresultobservation") #"patientnoteproblem,patientnoteresultobservation")
dbutils.widgets.text("EXPERIMENT_ID", defaultValue="1504400005030765")
dbutils.widgets.text("OUTPUT_TABLE", defaultValue="edav_dev_cdh.cdh_mimic_exploratory.note_clean")
...
documentNormalizerXML = ( DocumentNormalizer() 
                       .setInputCols(["note_raw"]) 
                       .setOutputCol("note_xml") 
                       .setAction("extract") 
                       .setPatterns(["name"])
                       .setReplacement("") 
...
documentNormalizerHTML = ( DocumentNormalizer() 
                       .setInputCols(["note_raw"]) 
                       .setOutputCol("note_html") 
                       .setAction("clean") 
                       .setPatterns(["<[^>]*>"])
                       .setReplacement("") 
                       .setPolicy("pretty_all")
...
documentNormalizerRTF = ( DocumentNormalizer() 
                       .setInputCols(["note_raw"]) 
                       .setOutputCol("note_rtf") 
                       .setAction("clean") 
                       .setPatterns([r"\{\*?\\[^{}]+}|[{}]|\\\n?[A-Za-z]+\n?(?:-?\d+)?[ ]?"])
                       .setReplacement("") 
                       .setPolicy("pretty_all")
...
removerPipeline = Pipeline().setStages([
    documentAssembler,
    documentNormalizerXML,
    documentNormalizerHTML,
    documentNormalizerRTF,
#    documentSentences,
...
textcols=["note_id","note_text"]
doc_table=doc.select(textcols+[F.explode(doc.note_xml.result).alias("xml_doc")])
display(doc_table)


# COMMAND ----------

#TODO posexplode() sentences

# COMMAND ----------


silver_df = (
    doc_table.transform(text_process.encoding_decider, "note_text", text_process.cdh_proposed)
...
).select("note_id","clean_text","encoding","word_count").orderBy("note_id")
