...
# MAGIC --  m.*
# MAGIC from
# MAGIC   edav_prd_cdh.cdh_truveta.conditioncodeconceptmap m
# MAGIC   where m.codeconceptid in 
# MAGIC   ( select distinct ct.conceptid from
# MAGIC     edav_prd_cdh.cdh_truveta.concept ct
# MAGIC ---    on m.codeconceptid = ct.conceptid
# MAGIC where ct.conceptcode  
# MAGIC             in ('Q87.11','Q90.0','Q90.1','Q90.2','Q90.9','Q91.0','Q91.1','Q91.2',
# MAGIC             'Q91.3','Q91.4','Q91.5','Q91.6','Q91.7','Q92.0','Q92.1','Q92.2',
...
# MAGIC              'Q87.2','Q87.3','Q87.5','Q87.81','Q87.82','Q87.89','Q89.7','Q89.8','R41.83') then 'IDDE'  
# MAGIC              end as IDD 
# MAGIC              from edav_prd_cdh.cdh_truveta.condition c 
# MAGIC              --All pts,all diagnosis in universal dx table 
# MAGIC              left join cdh_truveta.conditioncodeconceptmap m 
# MAGIC              on c.codeconceptmapid=m.id 
# MAGIC              left join cdh_truveta.concept ct 
...
# MAGIC              from edav_prd_cdh.cdh_truveta.condition c 
# MAGIC              --All pts,all diagnosis in universal dx table 
# MAGIC              left join edav_prd_cdh.cdh_truveta.conditioncodeconceptmap m 
# MAGIC              on c.codeconceptmapid=m.id 
# MAGIC              left join edav_prd_cdh.cdh_truveta.concept ct 
# MAGIC              on m.codeconceptid=ct.conceptid  ) as p  
# MAGIC ```
...
# MAGIC from
# MAGIC   edav_prd_cdh.cdh_truveta.conditioncodeconceptmap m
# MAGIC     join edav_prd_cdh.cdh_truveta.concept ct
# MAGIC       on m.codeconceptid = ct.conceptid
# MAGIC order by
# MAGIC   CodeSystem,
# MAGIC   ConceptCode
...
truveta_concept=spark.table("edav_prd_cdh.cdh_ml.concept_truveta")
display(truveta_concept)
...
# MAGIC select
# MAGIC   distinct m.CodeConceptId as ConceptId,
...
# COMMAND ----------
# DBTITLE 1,convert whole concept set to dictionary
#dict(sorted(my_dict.items()))
concept_set_dict={row["concept_code"]: row["concept_set"] for row in concept_set_df.collect()}
concept_set_dict=dict(sorted(concept_set_dict.items()))
print(concept_set_dict)
...
truveta_condition_df=spark.table("edav_prd_cdh.cdh_truveta.condition")\
  .withColumn('feature_name',concept_set_map[F.col("codeconceptmapid")])\
#  .orderBy('patientid','feature_name')
display(truveta_condition_df)

# COMMAND ----------

final_df=truveta_condition_df.withColumn("IDD",F.lit(1))\
  .groupby(F.col("patientId"))\

join_condition.write.mode("overwrite").saveAsTable("edav_prd_cdh.cdh_ml.join_condition")

# COMMAND ----------

final_df=join_condition.withColumn("IDD",F.lit(1))\
  .groupby(F.col("feature_name"))\
  .pivot("feature_name")\
  .sum("IDD")
