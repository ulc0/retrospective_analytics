...
i=spark.sql("""select conceptid from cdh_truveta.concept @v13 where conceptcode in ("1065667","1065692","260373001","720735008","10828004","52101004")""").createOrReplaceTempView("positivelab")

inv=spark.sql("""select conceptid from cdh_truveta.concept @v13 where conceptcode in ("1065712","1065714","1065719","1065717")""").createOrReplaceTempView("invalidlab")
...
cp=spark.sql("""
  with conid as (select conceptid 
   ...
   cond as (select personid,encounterid, recordeddatetime as dt
            from cdh_truveta.condition @v13
            where 1=1
            and codeconceptmapid in (select id from cdh_truveta.conditioncodeconceptmap@v13 where codeconceptid in (select conceptid from conid))
            ),

    medreq as (select personid,encounterid,authoredondatetime as dt 
            from cdh_truveta.medicationrequest@v13
            where 1=1
           and CodeConceptMapId in (select id from cdh_truveta.medicationcodeconceptmap@v13 where codeconceptid in (select conceptid from conid))
            ),

 medadm as (select personid,encounterid,recordeddatetime as dt 
            from cdh_truveta.medicationadministration@v13
            where 1=1
            and codeconceptmapid in (select id from cdh_truveta.medicationcodeconceptmap@v13 where codeconceptid in (select conceptid from conid))
            ),

  meddisp as (select personid,"" as encounterid, recordeddatetime as dt
            from cdh_truveta.medicationdispense@v13
            where 1=1
            and codeconceptmapid in (select id from cdh_truveta.medicationcodeconceptmap@v13 where codeconceptid in (select conceptid from conid))
            ),

  lab as (select personid,encounterid,effectivedatetime as dt
          from cdh_truveta.labresult@v13 l
         
          where 1=1
          and codeconceptmapid in (select id from cdh_truveta.labresultcodeconceptmap@v13 where codeconceptid in (select conceptid from conid))
  ...
  obs as (select personid,encounterid, recordeddatetime as dt
            from cdh_truveta.observation@v13
            where 1=1
            and codeconceptmapid in (select id from cdh_truveta.observationcodeconceptmap@v13 where codeconceptid in (select conceptid from conid))
        ),
  
  proc as (select personid,encounterid, recordeddatetime as dt
          from cdh_truveta.procedure@v13
          where 1=1
          and codeconceptmapid in (select id from cdh_truveta.procedurecodeconceptmap@v13 where codeconceptid in (select conceptid from conid))
        )
...
region as (select personid,
         ...
         from cdh_truveta.personlocation@v13 pl                                    
         left join cdh_truveta.location@v13 lc on  pl.locationid=lc.id
         left join cdh_truveta.concept@v13 st on st.conceptid=lc.stateorprovinceconceptid
         ),
...
eth_gen as (select  e.id,
          ...
          from cdh_truveta.person@v13 e
          left join cdh_truveta.concept@v13 ec on ec.conceptid = e.ethnicityconceptid
          left join cdh_truveta.personrace@v13 r on r.personid = e.id
          left join cdh_truveta.concept@v13 rc on rc.conceptid = r.raceconceptid
          left join cdh_truveta.concept@v13 sx on sx.conceptid=e.genderconceptid
...
adpax as (select distinct personid,dispensedatetime as meddate, 'pax' as meds
            from cdh_truveta.medicationdispense@v13
            where 1=1
            and codeconceptmapid in (select id from cdh_truveta.medicationcodeconceptmap@v13 where codeconceptid in (select conceptid from dconidpax))
...
admol as (select distinct personid,dispensedatetime as meddate, 'mol' as meds
            from cdh_truveta.medicationdispense@v13
            where 1=1
            and codeconceptmapid in (select id from cdh_truveta.medicationcodeconceptmap@v13 where codeconceptid in (select conceptid from dconidmol))
            and dispensedatetime between '2023-06-01' and getdate()
            union
...
adrem as (select distinct personid,dispensedatetime as meddate, 'rem' as meds
            from cdh_truveta.medicationdispense@v13
            where 1=1
            and codeconceptmapid in (select id from cdh_truveta.medicationcodeconceptmap@v13 where codeconceptid in (select conceptid from dconidrem))
            and dispensedatetime between '2023-06-01' and getdate()            
            union
