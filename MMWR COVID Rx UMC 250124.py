...
heart=spark.sql("""
with conid as (
select conceptid from cdh_truveta.concept@v13 
where 1=1
...
cdisab = spark.createDataFrame(disab,col).withColumn('tp', F.lit('uni')).createOrReplaceTempView("disab_T")

disab=spark.sql("""
with conid as (
select conceptid from cdh_truveta.concept 
where 1=1
and codesystem in ('NDC','CPT','CVX','RxNorm','ICD10CM','ICD10PS','ICD9','LOINC','HCPCS','SNOMEDCT') 
...
select conceptid from cdh_truveta.concept 
where 1=1
and codesystem in ('NDC','CPT','CVX','RxNorm','ICD10CM','ICD10PCS','ICD9') 
