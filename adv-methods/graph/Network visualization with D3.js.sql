...
-- MAGIC %python
-- MAGIC stbl="edav_dev_cdh_test.dev_cdh_ml_test.tweets"
-- MAGIC import pandas as pd
-- MAGIC twits="twitter.csv"
-- MAGIC passtweets=pd.read_csv(twits)
-- MAGIC #tweets=spark.read.csv(twits)
-- MAGIC tweets=spark.createDataFrame(passtweets)
-- MAGIC tweets.write.mode("overwrite").option("overwriteSchema", "true").format("delta").saveAsTable(stbl)
...
create or replace view hashtags as
  ...
  from (
    ...
    from edav_dev_cdh_test.dev_cdh_ml_test.tweets
  )
