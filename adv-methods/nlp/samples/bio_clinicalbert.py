...
# MAGIC   source = "abfss://cdh@davsynapseanalyticsdev.dfs.core.windows.net/machine_learning/",
# MAGIC   mount_point = "/mnt/cdhml",
...
output_schema = "ml_unstructured_analysis"
...
thisModel="emilyalsentzer/Bio_ClinicalBERT"
thisPipeline="text-classification"
tokenizer = AutoTokenizer.from_pretrained(thisModel)
model = AutoModel.from_pretrained(thisModel, device=device)

#summarizer = pipeline("summarization", model=thisModel, device=device)
##for thisPipeline in nlp_task:
#  print(thisPipeline)
#  clinical = pipeline(thisPipeline, model=thisModel, device=device)
...
class SummarizationPipelineModel(mlflow.pyfunc.PythonModel):
  def load_context(self, context):
    device = 0 if torch.cuda.is_available() else -1
    self.pipeline = pipeline(thisPipeline, context.artifacts["pipeline"], device=device)
...
pipeline_path="abfss://cdh@davsynapseanalyticsdev.dfs.core.windows.net/exploratory/databricks_ml/"

pipeline_path="/mnt/cdhml"
