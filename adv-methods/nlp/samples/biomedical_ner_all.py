...
# MAGIC
# MAGIC https://learn.microsoft.com/en-us/azure/databricks/machine-learning/train-model/huggingface/model-inference-nlp
# MAGIC
...
# MAGIC
# MAGIC [Source](https://docs.databricks.com/_extras/notebooks/source/deep-learning/hugging-face-transformers-batch-nlp.html)
# MAGIC ## Cluster setup
...
# MAGIC %md
# MAGIC [HUGGINGFACE_HUB_CACHE](https://huggingface.co/docs/huggingface_hub/v0.15.1/guides/manage-cache#limitations)
# MAGIC
...
import os
import torch
hasGpu = 0 if torch.cuda.is_available() else -1

hfModel="d4data/biomedical-ner-all"
tokenizer = AutoTokenizer.from_pretrained(hfModel)
model = AutoModelForTokenClassification.from_pretrained(hfModel)
clinical = pipeline("ner", model=model, tokenizer=tokenizer, aggregation_strategy="simple",device=hasGpu) # pass device=0 if using gpu
...
@pandas_udf('string')
def ner_batch_udf(texts: pd.Series) -> pd.Series:
#  pipe = tqdm(clinical(texts.to_list(), truncation=True, batch_size=8), total=len(texts), miniters=10)
  pipe = tqdm(clinical(texts, batch_size=8), total=len(texts), miniters=10)
  results = [json.dumps(ner) for ner in pipe]
...
class ClinicalNERModel(mlflow.pyfunc.PythonModel):
  def load_context(self, context):
    device = 0 if torch.cuda.is_available() else -1
    self.pipeline = pipeline(clinical, context.artifacts["pipeline"], device=device)
...
# or use the artifacts path + pipeline?
# nope pipeline_path="dbfs:/databricks/mlflow-tracking/f935b0552741449583e0834b0ae0478c/pipeline/"
artifact_path="clinical_ner_mode"
clinical.save_pretrained(pipeline_path)
with mlflow.start_run() as run:   
  #mlflow.doctor()     
  mlflow.pyfunc.log_model(artifacts={'pipeline': pipeline_path}, 
                          artifact_path=artifact_path, 
                          python_model=ClinicalNERModel())
...
loaded_model = mlflow.pyfunc.spark_udf(spark, model_uri=logged_model_uri, result_type='string') #,env_manager="conda")
