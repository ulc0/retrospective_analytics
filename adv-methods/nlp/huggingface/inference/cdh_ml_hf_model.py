...
dbutils.widgets.text('model_name', defaultValue='alvaroalon2/biobert_diseases_ner')
architecture = dbutils.widgets.get("model_name")
model=architecture.split('/')[1].replace('-',"_")
print(model)
# COMMAND ----------


dbutils.widgets.text('input_table_name' , 
    defaultValue="edav_prd_cdh.cdh_abfm_phi_exploratory.aix_feedback_note_bronze_mpox")
...
dbutils.widgets.text('output_table' , 
    defaultValue=f"{input_table_name}_{model}_{aggregation}")
...
bsize=64 #needs to be optimized for GPU
if (device==-1):
    bsize=1
print(bsize)
print(device)
...
tokenizer = transformers.AutoTokenizer.from_pretrained(architecture, stride =50, return_overflowing_tokens=True, model_max_length=512, truncation=True, is_split_into_words=True)
...
hf_pipe = (
    transformers.pipeline(task="ner",
             model=architecture,
