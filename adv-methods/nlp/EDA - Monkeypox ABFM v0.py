...
visit  = spark.table("cdh_abfm_phi.visit")display(visit_diagnosis_cleaned)
...
collected_notes = spark.read.table("cdh_abfm_phi_exploratory.mpox_data_set_ulc0")
...
#spark.sql("""DROP TABLE IF EXISTS cdh_abfm_phi_exploratory_ulc0_{}""".format(version_run1))
#mpox_notes.write.mode("overwrite").format("parquet").saveAsTable(f"{db}.ulc0_{version_run1}")

# COMMAND ----------

spark.table(f"{db}.ulc0_{version_run1}").display()
spark.table(f"{db}.ulc0_{version_run1}").count()
...
#spark.sql("""DROP TABLE IF EXISTS cdh_abfm_phi_exploratory_ulc0_{}""".format(version_run2))
#notes.write.mode("overwrite").format("parquet").saveAsTable(f"{db}.ulc0_{version_run2}")

# COMMAND ----------

spark.table(f"{db}.ulc0_{version_run2}").display()
spark.table(f"{db}.ulc0_{version_run2}").count()


# COMMAND ----------

f"{db}.ulc0_{version_run2}"
