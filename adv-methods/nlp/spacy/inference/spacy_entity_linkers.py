...
# MAGIC %md
# MAGIC install /Volumes/edav_dev_cdh/cdh_ml/compute/packages/wheels/en_core_sci_lg-0.5.4.tar.gz
# MAGIC %pip freeze
...
dbutils.widgets.text('OUTPUT_TABLE' , defaultValue= f"edav_dev_cdh.cdh_mimic_exploratory.note_clean_{MODEL_NAME}")
...
nlp.add_pipe(
    "scispacy_linker", config={"resolve_abbreviations": False, "linker_name": f"{LINKER_NAME}"}
)
   ...
   # entity_details.append(entity)
    ...
    try:
        for linker_ent in entity._.kb_ents:
            links.append(linker_ent)
            #print(f"linker_ent: {linker_ent}")
