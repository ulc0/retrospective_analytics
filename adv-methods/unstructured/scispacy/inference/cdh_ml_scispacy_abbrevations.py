...
dbutils.widgets.text('INPUT_TABLE_NAME' , defaultValue="edav_prd_cdh.cdh_abfm_phi_ra.silver_notes")
...
# MAGIC ```
# MAGIC import spacy
# MAGIC from utilities.abbreviation import AbbreviationDetector
# MAGIC
# MAGIC nlp = spacy.load("en_core_web_sm")
# MAGIC nlp.max_length = 43793966
# MAGIC
# MAGIC abbreviation_pipe = AbbreviationDetector(nlp)
# MAGIC nlp.add_pipe(abbreviation_pipe)
# MAGIC
# MAGIC text = [nlp(text, disable = ['ner', 'parser','tagger']) for text in train.text]  <---- disable parser, tagger, ner?
# MAGIC
# MAGIC text = ' '.join([str(elem) for elem in text]) 
# MAGIC
# MAGIC
# MAGIC doc = nlp(text)
# MAGIC
# MAGIC #Print the Abbreviation and it's definition
# MAGIC print("Abbreviation", "\t", "Definition")
# MAGIC for abrv in doc._.abbreviations:
# MAGIC     print(f"{abrv} \t ({abrv.start}, {abrv.end}) {abrv._.long_form}")
# MAGIC ```
