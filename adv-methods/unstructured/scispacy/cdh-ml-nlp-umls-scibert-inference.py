...
# MAGIC
# MAGIC ## Instructions:
# MAGIC 1. Run the notebook against a cluster with Databricks ML Runtime version 13.3.x-gpu, to best re-create the training environment.
...
model_name = "spacy_scibert_umls"

# COMMAND ----------

# MAGIC %md
# MAGIC ## Environment Recreation
# MAGIC Run the notebook against a cluster with Databricks ML Runtime version 13.3.x-gpu, to best re-create the training environment.. The cell below downloads the model artifacts associated with your model in the remote registry, which include `conda.yaml` and `requirements.txt` files. In this notebook, `pip` is used to reinstall dependencies by default.
# MAGIC
...
model_name = "spacy_scibert_umls"
input_table_name = "cdh_abfm_phi_exploratory.ml_nlp_mpox_notes_entities"
output_table_path = "/FileStore/batch-inference/spacy_scibert_umls"
...
# MAGIC ```python
# MAGIC output_table = "" # Example: "ml.batch-inference.spacy_scibert_umls"
