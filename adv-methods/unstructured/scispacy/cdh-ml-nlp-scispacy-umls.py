...
CustomPath="/Volumes/edav_dev_cdh_test/dev_cdh_ml_test/data/linker/umls"
CustomCandidateGenerator = CandidateGenerator(
    ann_index=f"{CustomPath}/nmslib_index.bin",  # noqa
    tfidf_vectorizer=f"{CustomPath}/tfidf_vectorizer.joblib",  # noqa
    tfidf_vectors=f"{CustomPath}/tfidf_vectors_sparse.npz",  # noqa
    concept_aliases_list=f"{CustomPath}/concept_aliases.json",  # noqa
)
...
linker = EntityLinker(candidate_generator=CustomCandidateGenerator,
    resolve_abbreviations=False,)
linker.threshold = threshold
nlp.add_pipe(
            "scispacy_linker",
            config={"resolve_abbreviations": False, "linker_name":linker},
        )
