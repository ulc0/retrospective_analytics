...
# MAGIC %pip install johnsnowlabs
...
tokenizer = AutoTokenizer.from_pretrained("raynardj/ner-disease-ncbi-bionlp-bc5cdr-pubmed")
model = AutoModelForTokenClassification.from_pretrained("raynardj/ner-disease-ncbi-bionlp-bc5cdr-pubmed")
...
modelname = "MilosKosRad/BioNER"  # modelpath
tokenizer = AutoTokenizer.from_pretrained(modelname)  ## loading the tokenizer of the model
ner = pipeline(task="ner",model=modelname, tokenizer=tokenizer)
ner(text, aggregation_strategy="first")
...
visit  = spark.table("cdh_abfm_phi.visit")
...
pxNoteResultObs = spark.table("cdh_abfm_phi.patientnoteresultobservation")

#pxProcedure =spark.table("cdh_abfm_phi.patientprocedure")
pxProm = spark.table("cdh_abfm_phi.patientproblem")
pxNoteProb = spark.table("cdh_abfm_phi.patientnoteproblem")

pxNote = spark.table("cdh_abfm_phi.patientnote")
pxImunizaion = spark.table("cdh_abfm_phi.patientimmunization")

pxResObs = spark.table("cdh_abfm_phi.patientnoteresultobservation")
...
display(
    visits_STI
    #.select("patientuid")
    .where(F.col("diag_or_proc_code").isin(mpox_codes)        
           )
    .sort("patientuid","STI_encounter")
    .groupBy("patientuid")
    .agg(
        F.collect_list("STI_encounter").alias("dates")
        )
)
...
visit_STI_record = (
    ...
    .join(
        visit_diagnosis_cleaned # Joining any vist prior mpox or STI encounter. It could be anything else
        .select("patientuid", 
    ...
    .where(F.datediff(F.col("STI_encounter"),F.col("service_date")).between(0,15))
...
targetPopulation_draft = (
    visit_STI_record.unionByName(patientProblemRecord).dropDuplicates()
)
targetPopulation_draft.cache()
#display(targetPopulation)

# COMMAND ----------

index_mpox = (
    targetPopulation_draft.where("mpox_visit == 1")
    .groupBy(
        "patientuid","diag_or_proc_code","practiceid","STI_other_visit","mpox_visit","socio_economic_char"
        )
    .agg(
        (F.collect_list("STI_encounter").getItem(0)).alias("STI_encounter"),
        (F.collect_set("problemcomment").getItem(0)).alias("problemcomment")
    )
    .withColumn("mpox_date", F.col("STI_encounter"))
)
...
non_mpox_diag = targetPopulation_draft.where("mpox_visit = 0")
display(non_mpox_diag)
non_mpox_diag.select("patientuid").dropDuplicates().count()

# COMMAND ----------

targetPopulation = (
    index_mpox.unionByName(non_mpox_diag)
    .withColumn("problemcomment", F.lower("problemcomment"))
    .dropDuplicates()
)
display(
    targetPopulation
...
from_sti_mpox = ( # need to assign the label to mpox = 1 for patiens who had an sti visit and later developed mpox, need to check visit is before 15 of mpox diag
    targetPopulation
    .groupBy("patientuid")
    .agg(
        (F.array_distinct(F.collect_list("mpox_visit"))).alias("mpox_visit2"),
        (F.collect_list("mpox_date").getItem(0)).alias("mpox_date2")
    )
    .withColumn("arr_size",
                F.size("mpox_visit2")
                )
    .sort(F.col("arr_size").desc())
    .select("patientuid",(to_date(F.col("mpox_date2"),"yyyy-MM-dd")).alias("mpox_date2"))
    .where("arr_size > 1")
)

display(from_sti_mpox)

# COMMAND ----------

# comparing px that went from mpox o to mpox 1, is there any with an STI visit within 15 days? If the answer is no, then I do not need to change the label to account for the incubation period of 15 days

targetPopulation2_check = (
    targetPopulation
    .join(from_sti_mpox,["patientuid"],'left')
    .withColumn("date_diff",
                F.datediff(F.col("mpox_date2"),F.col("STI_encounter"))
                )
    .filter("date_diff between 0 and 15")
    .drop("mpox_date2","date_diff")    
)

display(targetPopulation2_check)

# COMMAND ----------

#identify mpox index diagnosis
# identify additional visits <15 days before index diag
# for earch sti visit, bring visit date <15 days before that STI visit. Finding additional STI visits related in multiple tables
# 

# COMMAND ----------

targetPopulation.count()

# COMMAND ----------

targetPopulation.select("patientuid").where("mpox_visit = 1").dropDuplicates().count()

# need to check how many patients changed from non-mpox to mpox, then check how many within 15 days...
...
"""case_contol =  (
    ...
    .agg(
        ...
        F.array_distinct(F.collect_list("problemcomment")).alias("diagnosistext"),
        F.array_distinct(F.collect_list("practiceid")).alias("practiceids"),
        (F.max("STI_other_visit")).alias("STI_other_visits"), # this line does nothing, keeping it for schema issues later on
        (F.array_max(F.collect_list("mpox_visit"))).alias("exposed"),     
    )
)    
display(case_contol)
...
case_contol =  (
    targetPopulation
    .dropDuplicates(["patientuid","STI_encounter","problemcomment","mpox_date"])  
    .withColumn("exposed",
                F.when(F.col("mpox_date").isNotNull(),1).otherwise(0)
    )  
    .join(patientData.select("patientuid","age_group","gender"),["patientuid"]
          )
)    
#display(case_contol)
#patientData.printSchema()
...
df_exposed = (
    case_contol
    .withColumnRenamed("mpox_date","mpox_dates")    
    .where("exposed = 1")
...
# MAGIC %md ### bring notes here, so I can have more details on the controls. For the cases there is no much I can do
...
df_non_exposed_note = (
    df_non_exposed
    .join(
        patient_notes,['patientuid'], 'left' 
          )
    .where(F.datediff(F.col("STI_encounter"),F.col("note_date"))<=15)
    .withColumn("notes", concat_ws(" ", F.col("notes"), F.col("problemcomment")))
    .withColumn("note_size",
               F.length(F.col("notes"))
               )
)
#display(df_non_exposed_note)
...
unexposed_person_number = (    
    df_non_exposed_note    
        .select(
            ...
            "note_size",  
            F.row_number().over(
                Window.partitionBy("age_group", "gender","mpox_exposure_month_ne").orderBy(F.col("note_size").desc(),F.expr("uuid()"))
...
# discontinuis this table the one above seems superior.
"""pxNote_filtered = (
    pxNote
    .select(
        "patientuid",        
...
case_cohort_notes = (
    ...
    .where(
        ((F.datediff(case_cohort_matched.mpox_dates, patient_notes.note_date)).between(1,16)) # looking at notes in a day before the index diagnosis and earlier than that
        #& 
        #(F.length(F.col("notes")) >= 50)
        )
    .where(
        ((F.datediff(F.col("STI_encounter"),F.col("note_date"))).between(1,16)) 
        | ((F.datediff(F.col("STI_encounter"),F.col("mpox_dates"))).between(1,16))
    )
    .withColumn("mpox_dates",
                F.when(F.col("exposed") == 1, F.col("mpox_dates")).otherwise(None)
                )  
)
...
DB_NAME= 'mpox_data_set_presentation_intermediate_table'
USER = 'run9'
...
collected_notes = (
    spark.read.table("cdh_abfm_phi_exploratory.mpox_data_set_run9")
...
mpox_label = (
    ...
    .groupBy("patientuid")
    .agg(
        F.collect_list("STI_encounter").alias("STI_encounter"),
        (F.array_max(F.collect_set("mpox_exposed"))).alias("mpox_exposed")
    )
    .withColumnRenamed("mpox_exposed", "relabel")
    .select("patientuid","relabel")
    .where("mpox_exposed = 1")
)
...
final_data_set = (
    collected_notes
    .join(mpox_label, ["patientuid"], 'left')
    .dropDuplicates()
    .withColumn("mpox_exposed",
                F.when(F.col("relabel") == 1, F.col("relabel")).otherwise(F.col("exposed"))
                )
    .drop("relabel")
...
DB_EXPLORATORY = 'cdh_abfm_phi_exploratory'
DB_NAME= 'mpox_data_set_presentation'
USER = 'run9'
...
# MAGIC %sql 
# MAGIC
# MAGIC select * from cdh_abfm_phi_exploratory.mpox_notes_txt sort by patient_id
...
# MAGIC %sql 
# MAGIC select encounterdate,note,sectionname,group1,group2,group3,group4 
# MAGIC from cdh_abfm_phi.patientnote 
# MAGIC where patientuid="E7C169BB-09B5-44AB-814B-E6502ED8C190"

# COMMAND ----------

# MAGIC %sql 
# MAGIC select encounterdate,	note_DEID
# MAGIC from cdh_abfm_lds.patientnote 
# MAGIC where patientuid="E7C169BB-09B5-44AB-814B-E6502ED8C190"
...
# MAGIC %sql 
# MAGIC select person_id, note_datetime, note_title,  provider_id, note_document  from cdh_abfm_phi_exploratory.ml_fs_abfm_notes_utf
...
test_UMLS= spark.table("cdh_abfm_phi_exploratory.ml_nlp_text_entities")
