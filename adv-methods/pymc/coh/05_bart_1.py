...
dat_tmp = spark.read.table("bart_pcc.end005_ccsr_smp1")
...
# MAGIC %r
# MAGIC uniq = unique(cbind(time, delta, xtrain))
# MAGIC uniq2 = rbind(uniq, uniq)
# MAGIC nuniq = nrow(uniq)
# MAGIC uniq2[,"covid_patient"] = c(rep(0,nuniq),rep(1,nuniq))
# MAGIC
# MAGIC uniq_pre = surv.pre.bart(uniq2[,"time"], uniq2[,"delta"], uniq2[,c("covid_patient", "age_bin", "gendr", "io_flag", "race")], x.test=uniq2[,c("covid_patient", "age_bin", "gendr", "io_flag", "race")])

# COMMAND ----------

# MAGIC %r
# MAGIC pred = predict(out, uniq_pre$tx.test)
...
# MAGIC %r
# MAGIC nr2 = length(pred$surv.test.mean)
# MAGIC tn2 = max(pred$times)
# MAGIC nn2 = nr2/tn2
# MAGIC
# MAGIC # mean
# MAGIC surv_mn2 = t(matrix(pred$surv.test.mean, ncol=nn2, nrow=tn2))
# MAGIC prob_mn2 = t(matrix(apply(pred$prob.test, 2, mean), ncol=nn2, nrow=tn2))
# MAGIC
# MAGIC #quantiles
# MAGIC surv_952 = apply(pred$surv.test, 2, quantile, probs=0.95)
# MAGIC surv_952 = t(matrix(surv_952, ncol = nn2, nrow = tn2))
# MAGIC
# MAGIC surv_052 = apply(pred$surv.test, 2, quantile, probs=0.05)
# MAGIC surv_052 = t(matrix(surv_052, ncol = nn2, nrow = tn2))
...
# MAGIC # cv_idx = xtrain[,"covid_patient"]==1
# MAGIC # ncv_idx = xtrain[,"covid_patient"]==0
# MAGIC cv_idx2 = pred$tx.test[pred$tx.test[,"t"]==1,][,"covid_patient"]==1
# MAGIC ncv_idx2 = pred$tx.test[pred$tx.test[,"t"]==1,][,"covid_patient"]==0
# MAGIC
...
# MAGIC %r
# MAGIC svs2 = list(
# MAGIC   apply(surv_mn2[ncv_idx2,], 2, mean),
# MAGIC   apply(surv_mn2[cv_idx2,], 2, mean),
# MAGIC   apply(surv_952[ncv_idx2,], 2, mean),
# MAGIC   apply(surv_952[cv_idx2,], 2, mean),
# MAGIC   apply(surv_052[ncv_idx2,], 2, mean),
# MAGIC   apply(surv_052[cv_idx2,], 2, mean)
# MAGIC )
...
# MAGIC %r
# MAGIC uniq = unique(cbind(time, delta, xtrain))
# MAGIC uniq3 = rbind(uniq, uniq,uniq,uniq)
# MAGIC nuniq = nrow(uniq)
# MAGIC uniq3[,"age_bin"] = c(rep(0,nuniq),rep(1,nuniq), rep(2,nuniq), rep(3, nuniq))
# MAGIC
# MAGIC uniq_pre2 = surv.pre.bart(uniq3[,"time"], 
# MAGIC                   uniq3[,"delta"], 
# MAGIC                   x.train=uniq3[,c("covid_patient", "age_bin", "gendr", "io_flag", "race")], 
# MAGIC                   x.test=uniq3[,c("covid_patient", "age_bin", "gendr", "io_flag", "race")])

# COMMAND ----------

# MAGIC %r
# MAGIC pred2 = predict(out, uniq_pre2$tx.test)

# COMMAND ----------

# MAGIC %r
# MAGIC # surv_mn3 = t(matrix(pred2$surv.test.mean, ncol=nn2, nrow=tn2))
# MAGIC # length(pred2$surv.test.mean)
# MAGIC # nn2

# COMMAND ----------

# MAGIC %r
# MAGIC nr3 = length(pred2$surv.test.mean)
# MAGIC tn3 = max(pred2$times)
# MAGIC nn3 = nr3/tn3
# MAGIC
# MAGIC # mean
# MAGIC surv_mn3 = t(matrix(pred2$surv.test.mean, ncol=nn3, nrow=tn3))
# MAGIC prob_mn3 = t(matrix(apply(pred2$prob.test, 2, mean), ncol=nn3, nrow=tn3))
# MAGIC
# MAGIC #quantiles
# MAGIC surv_953 = apply(pred2$surv.test, 2, quantile, probs=0.95)
# MAGIC surv_953 = t(matrix(surv_953, ncol = nn3, nrow = tn3))
# MAGIC
# MAGIC surv_053 = apply(pred2$surv.test, 2, quantile, probs=0.05)
# MAGIC surv_053 = t(matrix(surv_053, ncol = nn3, nrow = tn3))
# MAGIC
# MAGIC age0_idx3 = pred2$tx.test[pred2$tx.test[,"t"]==1,][,"age_bin"]==0
# MAGIC age1_idx3 = pred2$tx.test[pred2$tx.test[,"t"]==1,][,"age_bin"]==1
# MAGIC age2_idx3 = pred2$tx.test[pred2$tx.test[,"t"]==1,][,"age_bin"]==2
# MAGIC age3_idx3 = pred2$tx.test[pred2$tx.test[,"t"]==1,][,"age_bin"]==3
# MAGIC
# MAGIC svs3 = list(
# MAGIC   apply(surv_mn3[age0_idx3,], 2, mean),
# MAGIC   apply(surv_mn3[age1_idx3,], 2, mean),
# MAGIC   apply(surv_mn3[age2_idx3,], 2, mean),
# MAGIC   apply(surv_mn3[age3_idx3,], 2, mean),
# MAGIC   apply(surv_953[age0_idx3,], 2, mean),
# MAGIC   apply(surv_953[age1_idx3,], 2, mean),
# MAGIC   apply(surv_953[age2_idx3,], 2, mean),
# MAGIC   apply(surv_953[age3_idx3,], 2, mean),
# MAGIC   apply(surv_053[age0_idx3,], 2, mean),
# MAGIC   apply(surv_053[age1_idx3,], 2, mean),
# MAGIC   apply(surv_053[age2_idx3,], 2, mean),
# MAGIC   apply(surv_053[age3_idx3,], 2, mean)
# MAGIC )
...
# MAGIC hz_ncv95 = apply(prob_mn[ncv_idx,],2,quantile, probs=c(0.95))
# MAGIC hz_cv95 = apply(prob_mn[cv_idx,], 2, quantile, probs=c(0.95))
# MAGIC
# MAGIC # prob_mn[cv_idx,]/prob_mn[ncv_idx,]
# MAGIC # as.vector(prob_mn[cv_idx,])/as.vector(prob_mn[ncv_idx,])
# MAGIC
# MAGIC
# MAGIC print(hz_cv/hz_ncv)
# MAGIC print(mean(hz_cv/hz_ncv))
# MAGIC # print(mean(hz_cv95/hz_ncv95)) # idk if this is actually correct, i dont think it is
# MAGIC # print(mean(hz_cv05/hz_ncv05))
# MAGIC
...
# MAGIC %r
# MAGIC # check hazard ratio (approximate not actual due to not doing complete test)
# MAGIC hz_ncv2 = apply(prob_mn2[ncv_idx2,],2,mean)
# MAGIC hz_cv2 = apply(prob_mn2[cv_idx2,], 2, mean)
# MAGIC hz_ncv952 = apply(prob_mn2[ncv_idx2,],2,quantile, probs=c(0.95))
# MAGIC hz_cv952 = apply(prob_mn2[cv_idx2,], 2, quantile, probs=c(0.95))
# MAGIC hz_ncv052 = apply(prob_mn2[ncv_idx2,],2,quantile, probs=c(0.05))
# MAGIC hz_cv052 = apply(prob_mn2[cv_idx2,], 2, quantile, probs=c(0.05))
# MAGIC
# MAGIC
# MAGIC hr2 = prob_mn2[cv_idx2,]/prob_mn2[ncv_idx2,]
# MAGIC
# MAGIC hr_mn2 = apply(hr2, 2, mean)
# MAGIC hr_952 = apply(hr2, 2, quantile, probs=0.95)
# MAGIC hr_052 = apply(hr2, 2, quantile, probs=0.05)
# MAGIC
# MAGIC print(hr_mn2)
# MAGIC print(hr_952)
# MAGIC print(hr_052)
# MAGIC print(paste(mean(hr_052), mean(hr_mn2), mean(hr_952)))
# MAGIC # print(prob_mn2[cv_idx2,][2,])
# MAGIC # print(prob_mn2[ncv_idx2,][2,])
# MAGIC
# MAGIC # print(hz_cv2/hz_ncv2)
# MAGIC # print(mean(hz_cv2/hz_ncv2))
# MAGIC # print(mean(hz_cv952/hz_ncv952))
# MAGIC # print(mean(hz_cv052/hz_ncv052))

# COMMAND ----------

# MAGIC %r
# MAGIC # hz on age_bin
# MAGIC hr3 = prob_mn3[age1_idx3,]/prob_mn3[age0_idx3,]
# MAGIC
# MAGIC
# MAGIC hr_mn3 = apply(hr3, 2, mean)
# MAGIC hr_953 = apply(hr3, 2, quantile, probs=0.95)
# MAGIC hr_053 = apply(hr3, 2, quantile, probs=0.05)
# MAGIC
# MAGIC print(hr_mn3)
# MAGIC print(hr_953)
# MAGIC print(hr_053)
# MAGIC print(paste(mean(hr_053), mean(hr_mn3), mean(hr_953)))
...
# MAGIC
# MAGIC cph = coxph(Surv(time, delta) ~ covid_patient + age_bin + gendr + io_flag + race, data=data.frame(xtrain))
# MAGIC
...
# MAGIC # plot(pp_check, var="age_bin", ylim=c(-1,1), resid=F)
# MAGIC # plot(pp_check, var="gendr", ylim=c(-1,1), resid=F)
# MAGIC plot(pp_check, var="covid_patient",ylim=c(-1,1), resid=F)
# MAGIC # plot(pp_check, var="io_flag", ylim=c(-1,1), resid=F)
# MAGIC
# MAGIC
...
# MAGIC %r
# MAGIC plot(c(0,tt), c(1,svs2[[1]]), ylim=c(0,1), type="l", col="darkblue", ylab="suvival", xlab="time(month)")
# MAGIC lines(c(0,tt), c(1,svs2[[3]]), ylim=c(0,1), type="l", col="#99CCFF")
# MAGIC lines(c(0,tt), c(1,svs2[[5]]), ylim=c(0,1), type="l", col="#99CCFF")
# MAGIC lines(c(0,tt), c(1,svs2[[2]]), ylim=c(0,1), type="l", col="#FF0000")
# MAGIC lines(c(0,tt), c(1,svs2[[4]]), ylim=c(0,1), type="l", col="#FFCCCC")
# MAGIC lines(c(0,tt), c(1,svs2[[6]]), ylim=c(0,1), type="l", col="#FFCCCC")

# COMMAND ----------

# MAGIC %r
# MAGIC plot(c(0,tt), c(1,svs3[[1]]), ylim=c(0,1), type="l", col="#0066CC", ylab="suvival", xlab="time(month)")
# MAGIC # lines(c(0,tt), c(1,svs3[[3]]), ylim=c(0,1), type="l", col="#99CCFF")
# MAGIC # lines(c(0,tt), c(1,svs3[[5]]), ylim=c(0,1), type="l", col="#99CCFF")
# MAGIC lines(c(0,tt), c(1,svs3[[2]]), ylim=c(0,1), type="l", col="#FF0000")
# MAGIC # lines(c(0,tt), c(1,svs3[[4]]), ylim=c(0,1), type="l", col="#FFCCCC")
# MAGIC # lines(c(0,tt), c(1,svs3[[6]]), ylim=c(0,1), type="l", col="#FFCCCC")
# MAGIC lines(c(0,tt), c(1,svs3[[3]]), ylim=c(0,1), type="l", col="#66FF00")
# MAGIC lines(c(0,tt), c(1,svs3[[4]]), ylim=c(0,1), type="l", col="#FFF000")
# MAGIC
...
# MAGIC %md
# MAGIC # todo
# MAGIC load in python and do the sksurv evaluations
# MAGIC reload with a large cut of the data
