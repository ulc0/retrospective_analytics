...
# MAGIC %md
# MAGIC def GetDistCPUCount():
# MAGIC     nWorkers = int(spark.sparkContext.getConf().get('spark.databricks.clusterUsageTags.clusterTargetWorkers'))
# MAGIC     GetType = spark.sparkContext.getConf().get('spark.databricks.clusterUsageTags.clusterNodeType')
# MAGIC     GetSubString = pd.Series(GetType).str.split(pat = '_', expand = True)
# MAGIC     GetNumber = GetSubString[1].str.extract('(\d+)')
# MAGIC     ParseOutString = GetNumber.iloc[0,0]
# MAGIC     WorkerCPUs = int(ParseOutString)
# MAGIC     nCPUs = nWorkers * WorkerCPUs
# MAGIC     return nCPUs
# MAGIC
# MAGIC ncores=GetDistCPUCount()
# MAGIC print(ncores)
...
prob = rearrange_out(pred$prob.test, M, N, K, pred$times)
