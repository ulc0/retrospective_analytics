...
# MAGIC
# MAGIC tokenizer = AutoTokenizer.from_pretrained("medical-ner-proj/bert-medical-ner-proj")
# MAGIC model = AutoModelForTokenClassification.from_pretrained("medical-ner-proj/bert-medical-ner-proj")
...
# MAGIC ner = pipeline(task="ner",model=model, tokenizer=tokenizer)
# MAGIC ner(text, aggregation_strategy="first")

# COMMAND ----------

# MAGIC %md
# MAGIC # New addutuib 10/16/2023
# MAGIC
# MAGIC
...
# MAGIC tokenizer = AutoTokenizer.from_pretrained("medical-ner-proj/bert-medical-ner-proj")
# MAGIC model = AutoModelForTokenClassification.from_pretrained("medical-ner-proj/bert-medical-ner-proj")
# MAGIC pipe = pipeline("ner", model=model, tokenizer=tokenizer, aggregation_strategy="average") 
# MAGIC
# MAGIC @pandas_udf('array<struct<word string, entity_group string, score float, start integer, end integer>>')
# MAGIC def ner_udf(texts: pd.Series) -> pd.Series:
# MAGIC   return pd.Series(pipe(texts.to_list(), batch_size=1))
# MAGIC
# MAGIC
# MAGIC output_agg_avg_bert_medical_ner = (
# MAGIC     treated_notes.select("*", ner_udf(treated_notes.note_string).alias('entities'))
# MAGIC     .withColumn("exploded_ent", F.explode(F.col("entities")))
# MAGIC     .select("*",(F.lower(F.col("exploded_ent.entity_group"))).alias("entity_group"), F.col("exploded_ent.word"), F.col("exploded_ent.score"))    
# MAGIC )
# MAGIC
# MAGIC #display(output_agg_avg_bert_medical_ner)
# MAGIC
...
def load_spacy_model(model_name):
    if spacy_ner is NULL:
        print("Loading spacy model...")
        spacy_ner=spacy.load(model_name)
return spacy_ner

# decorator indicating that this function is pandas_udf
# and that it's gonna process list of string
@pandas_udf(ArrayType(StringType()),model_name="en_ner_bc5cdr_md")
