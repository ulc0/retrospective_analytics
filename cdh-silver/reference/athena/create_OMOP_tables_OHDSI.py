...
# MAGIC from
# MAGIC  ${source_catalog}.${source_schema}.concept_cdh src
# MAGIC where
...
# MAGIC %sql
# MAGIC create
# MAGIC or replace table ${omop_catalog}.${omop_schema}.concept_xref as --select * from
# MAGIC --(
# MAGIC select
...
# MAGIC   replace(tar.vocabulary_id, 'CPT4', 'HCPCS') as tar_vocabulary_id,
...
# MAGIC   --                  )  as rn
# MAGIC from
# MAGIC   ${omop_catalog}.${source_schema}.concept_cdh src
...
# MAGIC   left join ${omop_catalog}.${source_schema}.concept_cdh tar on cr.concept_id_2 = tar.concept_id
...
# MAGIC   left join ${omop_catalog}.${source_schema}.concept_cdh val on crv.concept_id_2 = val.concept_id
...
# MAGIC   left join ${omop_catalog}.${omop_schema}.concept_relationship cr on src.concept_id = cr.concept_id_1
# MAGIC   and cr.relationship_id = 'Maps to'
# MAGIC   left join ${omop_catalog}.${source_schema}.concept_cdh tar on (
# MAGIC     cr.concept_id_2 = tar.concept_id
# MAGIC     and tar.standard_concept = 'S'
# MAGIC   ) --- this is a join of relationship maps to value ...
# MAGIC   left join ${omop_catalog}.${omop_schema}.concept_relationship crv on src.concept_id = crv.concept_id_1
# MAGIC   and crv.relationship_id = 'Maps to value'
# MAGIC   left join ${omop_catalog}.${source_schema}.concept_cdh val on (
...
# MAGIC --drop table if exists ${omop_catalog}.${omop_schema}.concept_procedure;
# MAGIC create
# MAGIC or replace table ${omop_catalog}.${omop_schema}.concept_condition as --select * from
# MAGIC --(
# MAGIC select
# MAGIC   src.code as src_code,
# MAGIC   -- replace(src.code, '.', '' ) as clean_code,
...
domain_list=['condition','procedure','observation','measurement',]
domain_dict={'condition':'%Cond%','procedure':'%Proc%','observation':'%Obs%','measurement':'%Meas%',}
# Dev, Drug

# COMMAND ----------

for domain in domain_list:
   domain_like=domain_dict[domain]
   antiSilver=f"Select * from {w['source_catalog']}.{w['source_schema']}.concept_xref s  left anti join  " \
     f"{w['omop_catalog']}.{w['omop_schema']}.concept_{domain} o on o.src_code=s.src_code " \
     f" and o.src_vocabulary_id=s.src_vocabulary_id where s.domain_id like '{domain_like}';"
   print(antiSilver)
...
# MAGIC from IDENTIFIER(concat(:source_catalog, '.', :source_schema, '.concept')) src
