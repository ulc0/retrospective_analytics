...
concept_tbl=f"{w['omop_catalog']}.{w['omop_schema']}.concept_cdh"
print(concept_tbl)
...
# MAGIC %md
# MAGIC ```python
# MAGIC # template for replacing column value with dictionary, two ways, two snippets
# MAGIC
# MAGIC deviceDict = {'Tablet':'Mobile','Phone':'Mobile','PC':'Desktop'}
# MAGIC
# MAGIC df['device_type'] = df['device_type'].replace(deviceDict,inplace=False)
# MAGIC
# MAGIC df_replace = df.replace(deviceDict,subset=['device_type'])
# MAGIC ```

# COMMAND ----------

import pyspark.sql.functions as F
concept_df=spark.table(f"{w['source_catalog']}.{w['source_schema']}.concept").filter(F.col("domain_id").isin(*domain_list))
concept_df.write.mode("overwrite").format("delta").option("overwriteSchema", "true").saveAsTable(f"{concept_tbl}")
...
tbl=concept_df.where(F.col('domain_id').isin(*domain_list)).groupby('domain_id','vocabulary_id').agg(F.count("concept_id").alias("concept_counts"))
display(tbl)
tbl=concept_df.groupby('domain_id','vocabulary_id').agg(F.count("concept_id").alias("concept_counts"))
display(tbl)
...
# MAGIC   --                  )  as rn
# MAGIC from
# MAGIC   ${omop_catalog}.{omop_schema}.concept_cdh src
