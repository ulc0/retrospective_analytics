...
data_list=spark.sql(f" select * from {CATALOG}.{SCHEMA}.umls_releases where current=='true'").toPandas()
display(data_list)

# COMMAND ----------

#data_product_list=[row.releaseType for row in data_list.select("releaseType").collect()]
data_product_list={d["releaseType"]:d["downloadUrl"] for d in data_list.to_dict(orient='records')}
print(data_product_list)
keys=list(data_product_list.keys())
print(keys)

# COMMAND ----------


dbutils.widgets.dropdown("DATA_PRODUCT","UMLS Full Release",keys)
dbutils.widgets.text("API_KEY","789be32d-9376-4611-a0d5-6e5ff6f7955c")
dbutils.widgets.text("DIRECTORY",f'/Volumes/{CATALOG}/{SCHEMA}/metadata_data/')

# COMMAND ----------

#Select Data Type, this should be parameterized at some point
# API_KEY will stop working if assignee does not sign into UMLS once every thirty days
...
UMLS_URI=data_product_list[DATA_PRODUCT]
#VERSION_URI=f"{DOWNLOAD_URI}{UMLS}&apiKey={API_KEY}"
VERSION_URI=f"{DOWNLOAD_URI}{UMLS_URI}&apiKey={API_KEY}"
print(VERSION_URI)

# COMMAND ----------

FILENAME=UMLS_URI.split('/')[-1]
print(FILENAME)

# COMMAND ----------

APIURI=f"{DOWNLOAD_URI}{UMLS_URI}&apiKey={API_KEY}"
print(APIURI)
