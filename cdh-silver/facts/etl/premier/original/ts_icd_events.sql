...
icd_pri_sec,icd_code as code, null as edate from ${DBCATALOG}.${SCHEMA}.paticd_diag 
union 
select pat_key,'N' as icd_poa, 'ICD10PCS' as vocabulary_id, icd_pri_sec, icd_code as code, 
...
timestamp(coalesce(edate,visit_period_startdate)) as startdate ,timestamp(coalesce(edate,visit_end_date)) as enddate, 
case 1=1
    when (t.icd_poa=='Y') then startdate when (t.icd_pri_sec=='A') then startdate else enddate 
end visit_start_date, 
visit_occurrence_number,vocabulary_id,code 
 FROM ts_premier_icd_preprocess t join  ts_encounter_index i 
on t.pat_key=i.visit_occurrence_number 
order by person_id,visit_occurrence_number,visit_start_date,code ;
...
-- MAGIC %md
-- MAGIC tbl \
-- MAGIC     .groupby(['person_id', 'episode_datetime','vocabulary_id','code']) \
-- MAGIC     .count() \
-- MAGIC     .where('count > 1') \
