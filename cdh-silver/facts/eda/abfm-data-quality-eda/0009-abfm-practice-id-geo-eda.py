...
def assign_regions(col_name='state_abbr'):
    
    abbr_col = col_name
    
    def _assign_regions(input_df):
        df = input_df.withColumn(
            'region',
            F.when(input_df[abbr_col].isin('WA','OR','ID','MT','WY','CA','NV','UT','CO','AZ','NM'),'west')
            .when(input_df[abbr_col].isin('ND','MN','WI','MI','SD','NE','KS','IA','MO','IL','IN','OH'),'midwest')
            .when(input_df[abbr_col].isin('OK','TX','AR','LA','KY','TN','MS','AL','WV','MD','DE','DC','VA','NC','SC','GA','FL'),'south')
            .when(input_df[abbr_col].isin('ME','VT','NH','MA','CT','RI','NY','PA','NJ'),'northeast')
        )
        return df
    return _assign_regions

# COMMAND ----------

display(
    states_df
    .transform(assign_regions())
)
