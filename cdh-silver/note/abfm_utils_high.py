...
import shared.etl_utils as etl_utils

def load_patient_table(data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.patient table."""
    import pyspark.sql.functions as F
    import pyspark.sql.windows as W
    ...
    #load data
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.patient{data_version}")
...
def load_generatedpatientbaseline_table(data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.generatedpatientbaseline table."""
    
    #load data
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.generatedpatientbaseline{data_version}")
    
    #transform data
    df = (
        df
        .transform(clean_date('dob'))
        .transform(clean_date('firstvisit'))
        .transform(clean_date('lastvisit'))
        .transform(clean_gender())
        .transform(clean_zipcode())
        .transform(clean_state('state'))
        .transform(create_age('dob','lastvisit'))
    )
...
def load_patientproblem_table(subset=True, data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.patientproblem table."""
    
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.patientproblem{data_version}")
...
def load_visit_table(subset=True, data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.visit table."""
    
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.visit{data_version}")
...
def load_visitdiagnosis_table(subset=True, data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.visitdiagnosis table."""
    
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.visitdiagnosis{data_version}")
...
def load_raceeth_table(subset=True, data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.generatedraceethnicity table."""
    
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.generatedraceethnicity{data_version}")
...
def load_patientimmunization_table(subset=True, data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.patientimmunization table."""
    
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.patientimmunization{data_version}")
...
def load_patientprocedure_table(subset=True, data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.patientprocedure table."""
    
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.patientprocedure{data_version}")
...
def load_patientlaborder_table(subset=True, data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.patientlaborder table."""
    
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.patientlaborder{data_version}")
...
def load_patientlanguage_table(subset=True, data_version=""):
    """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.patientlanguage table."""
    
    df = spark.table(f"edav_prd_cdh.cdh_abfm_phi.patientlanguage{data_version}")
...
# def load_{}_table():
#     """Returns a cleaned edav_prd_cdh.cdh_abfm_phi.{} table."""
    
#     df = spark.table("edav_prd_cdh.cdh_abfm_phi.{}")
