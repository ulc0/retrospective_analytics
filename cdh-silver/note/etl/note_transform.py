...
dbutils.widgets.text("NOTE_TABLE", defaultValue="edav_prd_cdh.cdh_abfm_phi_ra.note_bronze") #,patientnoteproblem,patientnoteresultobservation") #"patientnoteproblem,patientnoteresultobservation")
...
notes=spark.table(notetable) #.filter(F.col("provider_id").isin([2405,])).limit(10000)
display(notes)
...
doc=removerPipeline.fit(silver_df).transform(silver_df)
