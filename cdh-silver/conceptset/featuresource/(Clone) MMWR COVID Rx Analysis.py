...
# MAGIC Author: Erica Okene (contractor, GAP solutions) ulc0@cdc.gov
# MAGIC
...
positive_lab=["1065667","1065692","260373001","720735008","10828004","52101004"]
negative_lab=["1065712","1065714","1065719","1065717"]
pax=["2587899", "2587898", "2587892", "2587897", "2587893", "2587896", "2587894", "2587895", "2599543", "00069108530", "00069108506", "00069034506", "00069034530", "00069110104", "00069110120", "00069208501", "00069208502"]
...
f
result = {list_name: locals()[list_name]}
print(result)  # Output: {'my_list': [1, 2, 3, 4]}
...
""")
#cp.write.mode("overwrite").saveAsTable("/Workspace/Users/ulc0@cdc.gov/covid_"")
...
""")
gp.write.mode("overwrite").saveAsTable("cdh_truveta_exploratory.ulc0_truveta_lavacovidgp")
...
# MAGIC %sql
# MAGIC drop table if exists cdh_truveta_exploratory.ulc0_truveta_lavacovidgp

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from cdh_truveta_exploratory.ulc0_truveta_lavacovidgp
...
""").write.mode("overwrite").saveAsTable("cdh_truveta_exploratory.ulc0_truveta_lavacovidtp_meds")  
...
--COVID Dx and Lab Only. Exclude COVID Rx without diagnosis or lab.
             ...
             from cdh_truveta_exploratory.ulc0_truveta_lavacovidgp as tp 
             )
...
--Population Requirement: (Num) COVID diagnosis or lab only. Exclude COVID Rx without diagnosis or lab. 
      ...
      trt as (select distinct personid,meddate,concat(substr(meddate,1,4),substr(meddate,6,2),substr(meddate,9,2)) as medymd,medseason,meds 
             from cdh_truveta_exploratory.ulc0_truveta_lavacovidtp_meds
             ),
            
            umcon as (select distinct personid, dt,umc
            from cdh_truveta_exploratory.ulc0_truveta_MMWR_rx_cmc 
            where 1=1
            and umc IS NOT NULL)
...
""").write.mode('overwrite').saveAsTable('cdh_truveta_exploratory.ulc0_truveta_MMWR_rx_tpop')
#.write.mode('overwrite').option('mergeSchema', 'true').saveAsTable('cdh_truveta_exploratory.ulc0_truveta_MMWR_rx_tpop')  
...
# MAGIC %sql
# MAGIC drop table if exists cdh_truveta_exploratory.ulc0_truveta_MMWR_rx_tpop

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * from cdh_truveta_exploratory.ulc0_truveta_MMWR_rx_tpop
...
df_tot = spark.sql('''SELECT distinct agesi,
                   ...
                   from cdh_truveta_exploratory.ulc0_truveta_MMWR_rx_tpop
