...
for feature_code,ccsrList in featureCCSRDict.items():
    ...
    vlist="|".join(ccsrList)
    catcol=ccolumn[lt]
    sqlstring=f"select distinct LEFT(`{concept_column}`,3)||'.'||RIGHT(rjust(`{concept_column}`,10),2) as code, '{ct}' as vocabulary_id,'{fn}' as feature_name from {SRC_SCHEMA}.{ftable} where `{catcol}` RLIKE '{vlist}'"
    print(sqlstring) 
    values=spark.sql(sqlstring)
    display(values)
    ft_name=f"{{CATALOG}.SRC_SCHEMA}.concept_sets",
    ft=spark.table(ft_name)
