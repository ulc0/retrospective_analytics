...
# MAGIC # Background
# MAGIC
# MAGIC This work is based on the study "Browning, 2022. Evaluation of machine learning for predicting COVID-19 outcomes from a national electronic medical records database". The major objective of this research was to evaluate the feasibility of using EMR as potential indicators for predicting outcomes of COVID-19 inpatient encounters using the raw EMR data including 255 days prior and up to the first 24 hours of the first COVID-19 inpatinet encounter, with a major aspect of the aims being not to select predictor features *a priori* to modeling. 
# MAGIC
# MAGIC - Major outcomes predicted include escalation to ICU, MISA (hyperinflammatory syndrome) and death.
# MAGIC - Models used included regression, random forest, gradient boosting classifier, deep averaging network and recurrent neural network with LSTM
# MAGIC - Results indicated that all models performed equivalent on the full feature space with the best AUC determined being 79% found when predicing ICU admission. MISA was the most difficult to predict with best AUC being 70%.
# MAGIC
# MAGIC Prior to this verison, a implementation was completed evaluate the feasibility of ML/AI using the Databricks platform. That version was developed in conjuncture with many of the CDC's Databricks components being developed. While succesful in adequetly remplimenting the research, the prior implementaiton left several components untouched and a repository of files that was not conduitive to further evaluation, indicating a need to revise the work to provide a guide that covers the aspects of applying ML to Big Data using Databricks and the underlying Spark platforms.
# MAGIC
# MAGIC This current revision serves a purpose to consolidate several aspects of the prior edition and to provide a guide from which analyses using spark and Databricks components can drawn from. The aim is to give details on the utilites provide by Databricks and Spark to be used for new analysts using the CDC's Databricks platform for machine learning.

# COMMAND ----------

# MAGIC %md 
# MAGIC # Feature Extraction
# MAGIC This notebook covers the full feature exctraction/engineering workflow. The prior version had left this in the orignal studies "pandas-style" analyses and did not utilize any spark components throughout the work. While feature extraction and data cleaning are not necessarily Machine Learning components, we felt that reimplementation of this analyses using a spark based approach will help future users referencing this work as a guide be introduced to routine analytic components of data cleaning in spark.
# MAGIC
# MAGIC In this notebook each section into specific components and each section a brief description will be provided of the steps taken. Additionally, it will be seen **spark** notes will be provided throughout. These will give brief description of utilities for analyses using spark through the pyspark and sql APIs.
# MAGIC
# MAGIC ## Procedure
# MAGIC 1. Get Data
# MAGIC 2. Identify the Target Encounter
# MAGIC 3. Perform primary inclusion/exclusion of patients and relevant encounters
# MAGIC 4. Collect demographic features
# MAGIC 5. Collect encounter features
# MAGIC 6. Process encounter features
# MAGIC 7. Perform exclusions based on encounter features
# MAGIC 8. Perform final processing of features
# MAGIC 9. Save dataset

# COMMAND ----------

# MAGIC %md
# MAGIC Note on the code structure. Almost all pyspark operations are wrapped in python function. I have found this to be a simple way to manage the series of operations.
# MAGIC
# MAGIC Each section follows the structure:
# MAGIC - cell 1 for function writing
# MAGIC - cell 2 for function application
# MAGIC - cell 3 for checking output
# MAGIC
# MAGIC Additionally, throughout I aimed to explain pyspark/spark code structure as a general guide. These annotations are indicated such as `# 1.` and are explained in the **spark** text block
...
# MAGIC %md 
# MAGIC **spark**
# MAGIC - Databricks handles the process of setting up the spark instances. To start working with spark all is needed is to load the relevant modules.
# MAGIC - In this notebook the main modules come from `pyspark.sql` and include `functions`, `window`, `type` and `PandasUDFType`
# MAGIC - Common nomeclature is to name `functions` as "F", `Window` as "W" and `types` as "T" and call the methods through these objects. This protects namespaces of utilites as many methods/functions are named across modules.
...
# MAGIC %md
# MAGIC **spark**
# MAGIC 1. pypark conditions can be saved as seperate variables and the variables then inserted in the code
# MAGIC 2. dataframe operations **select** and **when**
# MAGIC   - pyspark performs operations on dataframes. A series of chained operations can be done through `.method()` chains. This is similar to pandas chaining or piping in R tidyver "%>%" 
# MAGIC     - multiline statements can be create by wrapping in "()"
# MAGIC   - `.select` is a process of selecting and creating variables
# MAGIC     - `.select("*"` indicates select all. Further variables can be created and added to this list
# MAGIC   - `F.` is the 'pyspark.sql.functions' module. 
# MAGIC   - `F.when` is similar to the R "ifelse" or "case_when()"
# MAGIC     - multiple "when" staements can be chained creating the case_when() similar operation
# MAGIC   - `.alias()` allows renaming to a created variable (otherwise it is given the name of the statement used to create it)
# MAGIC
# MAGIC 3. dataframe  operations **withColumn**, **Windows**
# MAGIC   - `withColumn` can be used to create a new variable (similar to "select").
# MAGIC     - similar to "mutate" in R tidyverse
# MAGIC     - a side note of withColumn, is that after each call, a new dataframe has to be returned. In some cases where many columns need to be created a "select" statement may be more optimal
# MAGIC   - `W` is the Window function access, allows grouping and sorting through none aggregate methods
# MAGIC     - `over(W.partitionBy())` is similar to R "group_by". "Over" is the actual tranformation function and "W.partition" creates the window. 
# MAGIC     - Typical function sequence is `withColumn("new_var", F.sql_func().over(W.partition("grouping_var"))`
# MAGIC     - `W.orderBy()` allows additional sorting of the window
# MAGIC     - Example:
# MAGIC       - `.withColumn("row",F.row_number().over(W.partitionBy(F.col("medrec_key"), F.col("incl_enc")).orderBy(F.col("days_from_index"))))`
# MAGIC       - Read as: create column named "row" as a row number for each row. For each group of "medrec_key"s and "incl_enc"s ordered by "days_from_idnex"
# MAGIC   - `F.col()` is the way to reference variables in a dataframe. There are some additional ways to do this that can additonaly explored.
# MAGIC   - `sdf.groupBy()` is the dataframe direct groupby statement. This is different than the window "groupBy" as it only allows further aggregrate transformations. Windows should be used when grouping and performing an operation on each observation is required.
# MAGIC 4. dataframe operations **Filter** and **Drop**
# MAGIC - `filter()` is similar to R "filter()"
# MAGIC - `drop()` is to remove variable columns from dataframe
...
# MAGIC %md
# MAGIC **spark**
# MAGIC 1. `.select("*", *[list or list comprension]`
# MAGIC   - a list can be passed to a "select" statement. Each element can be accessed using `*[list]` 
# MAGIC   - the additional list comprehension is a powerful structure that allows multiple columns to be created with minimal code written. However it is not always easily interpretable.
# MAGIC   - Example:
# MAGIC     - `.select("*",*[F.max(F.when(F.col("target") == 1, F.col(x))).over(W.partitionBy(F.col("medrec_key"))).alias(f"target_{x}") for x in misa_death])`
# MAGIC     - Read as - For each "item" in misa_death create a column named target_"item" as for each group in grouped by "medrec_key" get the max value from the set of values created by setting the set value to the "item" values when the "target" value equals 1. Select all and each of the created columns
# MAGIC     
...
# MAGIC %md
# MAGIC **spark**
# MAGIC 1. **Sample** and **Join**
# MAGIC   - `sample()` is dataframe function to get a sample from a spark dataframe
# MAGIC   - `join()` is dataframe function to perform join operations 
# MAGIC   
# MAGIC 2. **Cache**
# MAGIC   - Cacheing is the proces to save the output of a dataframe operation into memory. Without a cache, each dataframe operation just results in a instruction set telling spark how to create the dataframe. So a series of dataframe operations results in a series of instruction sets, starting from the initial data read in.
# MAGIC     - Considerations:
# MAGIC       - Cacheing can be helpful when developing a workflow and testing operations as it allows further operations to be built of the cached data, which can save time if there are expensive operations in the earlier steps. 
# MAGIC       - On consideration with cacheing dataframes is that is consume operation space for further steps, which can in turn slow down these steps, so cacheing is recommended to be done with some caution. 
# MAGIC       - A cache is only completed once an operation that returns results is completed such as `.count()` or `.show()`
# MAGIC     - In this step, only the sampled dataset will be cached. Since this dataset is relatively small, it will have no cost on further operations, but makes the next steps of the workflow faster as the primary steps no longer need to be completed.
# MAGIC   - Provided in the helpers at the top of the doc there is a function to help unpersist the cached dataframes.
# MAGIC   - It is recommended to read further into the background of spark and cacheing to gain deeper understanding of this method.
...
# MAGIC %md
# MAGIC **spark**
# MAGIC 1. **SQL Expressions**
# MAGIC - `F.expr("sql expression")` is an access point to using SQL specific functions not provided by the pyspark.sql.functions API
# MAGIC - They can be combined with other operations same as any other "F.func"
# MAGIC - Example:
# MAGIC   - `F.when(F.expr(f"any({hisp} = 'Y')").over(W.partitionBy(F.col("medrec_key"))), "Y")`
# MAGIC   - Read as - for each group, grouped by "medrec key", when any "hispanic_ind" in the group set equals "Y", then set as "Y"
# MAGIC
# MAGIC 2. **Collect_set** and **Flatten** and **Concat**
# MAGIC - `collect_set()` takes concept_set of items from a variable and turns them into a set as an array
# MAGIC - `flatten()` takes an observation of a nested array type and turns into a single array
# MAGIC - `concat()` combines multiple variables 
# MAGIC   - `concat_ws()` combines multiple variables, but with a deliminator between each
# MAGIC - Example:
# MAGIC   - `*[F.collect_set(F.concat(F.lit(f"{x} "), F.col(x))).over(w).alias(f"{x}{term}") for x in dem_lst]`
# MAGIC   - Read as - for each item in dem_list, create variable named "item_term". Create each as for group created by groupBy(W) collect the values in the group where the values are created created by the concatentation of "{x} and x" 
...
def get_ftrs(
    ...
    sel_cols = merge_cols(base_cols, text_col, res_col, replace_col, time_cols)
...
genlab_ftr = get_ftrs(sdf = genlab1,
                                   base_cols = ["pat_key"],
...
lab_res_temp = lab_res1.withColumn("lab_text", F.concat_ws(" ", F.col("test"), F.col("observation")))
