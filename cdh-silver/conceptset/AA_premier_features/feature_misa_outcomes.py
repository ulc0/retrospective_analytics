...
# MAGIC %md
# MAGIC # get pat_key of target encounters
# MAGIC target_key = target_enc.select("pat_key")

# COMMAND ----------

# MAGIC %md
# MAGIC target_key.count()

# COMMAND ----------

# MAGIC %md
# MAGIC def filt_targets(target, dtst):
# MAGIC     out = (target
# MAGIC            .join(dtst, on = "pat_key", how = "left")
# MAGIC           )
# MAGIC     return out
...
# MAGIC %md
# MAGIC # filter 
# MAGIC icd_enc = filt_targets(target_key, icd_enc)
# MAGIC proc_enc = filt_targets(target_key, proc_enc)
# MAGIC patbill = filt_targets(target_key, patbill)
# MAGIC vitals = filt_targets(target_key, vitals)
# MAGIC genlab = filt_targets(target_key, genlab)
# MAGIC labres = filt_targets(target_key, labres)

# COMMAND ----------

# MAGIC %md
# MAGIC print(icd_enc.select("pat_key").distinct().count())
# MAGIC print(proc_enc.select("pat_key").distinct().count())
# MAGIC print(patbill.select("pat_key").distinct().count())
# MAGIC print(vitals.select("pat_key").distinct().count())
# MAGIC print(genlab.select("pat_key").distinct().count())
# MAGIC print(labres.select("pat_key").distinct().count())

# COMMAND ----------

# MAGIC %md
# MAGIC print(icd_enc.count())
# MAGIC print(proc_enc.count())
# MAGIC print(patbill.count())
# MAGIC print(vitals.count())
# MAGIC print(genlab.count())
# MAGIC print(labres.count())
...
labs = (genlab.filter(
    (F.lower(F.col("lab_test_loinc_desc")).contains("reactive")))
       )
# labs.cache()
# labs.count()
...
# Ferriting
labs = (genlab.filter(
    (F.lower(F.col("lab_test_loinc_desc")).contains("sedimentation rate")) 
    ))
# labs.cache()
# labs.count()
...
# print(icd_enc.count())
# print(cc_cardiac.count())
# cc_cardiac.groupBy("icd_code").count().display()
...
# lab_6.groupBy("observation_day_number").count().display()
def get_bp_90_2(sdf):
    out = (sdf
           ...
           .withColumn("row", F.row_number()
                       .over(W.partitionBy("pat_key","observation_day_number").orderBy("observation_day_number")))
...
def get_chg_code(sdf, var, collect = False):
    out = (sdf
           .filter(F.lower(F.col("std_chg_desc")).contains(var))
          )
    if collect:
        out = list(out
               .select("std_chg_code")
...
clin_comp = (cardiac
             ...
             .withColumn("cc_ind",F.col("sum_cc")>=2)
...
misa_enc.write.mode("overwrite").format("delta").saveAsTable("poc_premier.outcomes_misa")
