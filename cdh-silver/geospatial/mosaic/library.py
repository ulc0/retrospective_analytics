...
def get_items(geojsons: pd.Series, date_times: pd.Series, concept_set: pd.Series) -> pd.Series:

  from tenacity import retry, wait_exponential

  @retry(wait=wait_exponential(multiplier=2, min=4, max=240))
  def search_with_retry(geojson, catalog, concept_set, dt):
    search = catalog.search(
        concept_set = concept_set,
        intersects = geojson,
        datetime = dt
      )
    items = search.item_concept_set()
    return [json.dumps(item.to_dict()) for item in items]

  def search_catalog(geojson, catalog, concept_set, dt):
    try:
      return search_with_retry(geojson, catalog, concept_set, dt)
  ...
  items = []
  for geojson, concept_set, date_time in zip(geojsons, concept_set, date_times):
    items.append(
      search_catalog(geojson, catalog, concept_set, date_time)
    )
  ...
  try:
    ...
    return (
      ...
      .repartition(repart_num)
        ...
        .withColumn("item_concept_set", F.col("item.concept_sets"))
