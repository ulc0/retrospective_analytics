...
#spark.sql("""DROP TABLE IF EXISTS edav_prd_cdh.cdh_abfm_phi_exploratory.note_silver_QC_9_9_24""")

# COMMAND ----------

#result_df_3.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(f"{catalog_name}.{args['schema']}.{args['source_table']}_QC_9_9_24")
...
f"{catalog_name}.{args['schema']}.{args['source_table']}_QC_9_9_24"
...
display(
  ...
  .where("note_id in ('58396', '25769864346', '34359796601')")
  )
...
# MAGIC %md ## Test cases information - sentence

# COMMAND ----------

cleaned_table = (
    spark.table("edav_dev_cdh.cdh_mimic_exploratory.note_clean")
    .limit(30)
)

sentence_table = (
    spark.table("edav_dev_cdh.cdh_mimic_ra.note_sentences")
)


whole_note_and_sentence = (
    cleaned_table
    .join(sentence_table, 
          ['note_id']          
          )
...
display(cleaned_table)
display(sentence_table)
display(whole_note_and_sentence)
