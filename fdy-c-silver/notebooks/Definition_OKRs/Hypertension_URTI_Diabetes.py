...
# Hypertension
HT = ['I10', 'I110', 'I119', 'I12', 'I120', 'I129', 'I13', 'I130', 'I131', 'I1310', 'I1311', 'I132', 'I15', 'I150', 'I151', 'I152', 'I158','I159', 'I16', 'I160', 'I161', 'I169'] 
...
visit_test = (
    visit.select('patientuid',F.col('encountertypecode').alias("diag_code"))
    .withColumn("diagnosis_label", condition
                )
    .filter(F.col("diagnosis_label").isNotNull())
    )
...
visit_diagnosis_cleaned_test = (
    visit_diagnosis_cleaned
    .withColumnRenamed("encounterdiagnosiscode","diag_code")
    .withColumn("diagnosis_label", condition
                )
    .filter(F.col("diagnosis_label").isNotNull())
...
selected_patients = (
    visit_diagnosis_cleaned_test.select("person_id").distinct().where("diagnosis_label = 'DM'").limit(10)
    .unionByName(visit_diagnosis_cleaned_test.select("person_id").distinct().where("diagnosis_label = 'URTI'").limit(10))
    .unionByName(visit_diagnosis_cleaned_test.select("person_id").distinct().where("diagnosis_label = 'HT'").limit(10))

)


display(visit_diagnosis_cleaned_test)
...
person_note_id = ( ## SELECTING ALL NOTES FROM Bronze table because we would like to see if any note would have the diagnosis or evidence or sign of DM, URTI or HT
    ...
    .distinct()
    .join(
        selected_patients, ['person_id']
        )
)

display(person_note_id)
person_note_id.select("person_id").distinct().count()

# COMMAND ----------

# loading NER tables (silver)

NER_table = (
    person_note_id
    #.join(
        # NOTE: we do not have a most current run for NER in silver where entities are identified correcly in ABFM
    #spark.table("edav_prd_cdh.cdh_abfm_phi_exploratory.aix_feedback_note_bronze_mpox_biomedical_ner_all") # using bronze bc the silver is still pending
    #spark.table("edav_prd_cdh.cdh_abfm_phi_exploratory.aix_feedback_note_bronze_mpox_bioformer_8l_ncbi_disease")
    #spark.table("edav_prd_cdh.cdh_abfm_phi_exploratory.aix_feedback_note_bronze_mpox_distilbert_base_uncased_ft_ncbi_disease")
...
dm_px = (
    visit_diagnosis_cleaned_test
    .where(F.col("diagnosis_label") == "DM")
...
ht_px =  (
    visit_diagnosis_cleaned_test
    .where(F.col("diagnosis_label") == "HT")
    .select("person_id","diagnosis_label")
    .distinct()
    .limit(5)
)
...
urti_px = (
    visit_diagnosis_cleaned_test
    .where(F.col("diagnosis_label") == "URTI")
    .select("person_id","diagnosis_label")
    .distinct()
    .limit(5)
)
