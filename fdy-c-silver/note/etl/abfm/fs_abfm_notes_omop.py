...
def notes_query(nschema, ntable):  # ,nbegin,nend):
    sqlstring1 = f" (select person_id as person_id,'{ntable}' as note_type,practiceid as provider_id, observation_datetime as note_datetime"
    sqlstring1 = f" (select person_id,provider_id, observation_datetime as note_datetime"
    ...
    ##sqlstring_predicate=f"where (observation_datetime)<={sbegin} and year(observation_datetime)<={send} UNION"
#no longer used
    sqlstring_predicate = (
        f" where person_id in ({cohort}) order by person_id, note_datetime) UNION"
    )
    #    sqlstring_predicate=f"(select distinct person_id from {nschema}.{ntable} where year(observation_datetime)<={nbegin} and year(observation_datetime)<={nend} ) order by person_id, observation_datetime UNION"
...
# MAGIC select distinct person_id,provider_id, observation_datetime as note_datetime,
