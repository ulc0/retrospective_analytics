...
demfact_discharge=dem_wide.melt(['person_id','discharge_datetime',],fact_concept_list,'vocabulary_id','concept_code').withColumnRenamed('discharge_datetime','observation_datetime')
...
demts=dem_wide.melt(['person_id','observation_datetime'],fact_concept_list,'vocabulary_id','concept_code')
display(demts)
...
deaths=demts.filter( (demts.vocabulary_id=='discharged_to_code') & (demts.concept_code.isin(death_discharge_codes))).drop('admission_datetime').drop('vocabulary_id').withColumnRenamed('discharge_datetime','observation_datetime').withColumn('vocabulary_id',F.lit('UB04PATIENTSTATUS'))
#.drop(['vocabulary_id','admission_datetime']).select(F.lit('DEATH').alias('vocabulary_id')).select('discharge_datetime').alias('observation_datetime').drop('discharge_date')
display(deaths)
