...
ccsr_lookup = spark.table("{CATALOG}.{SRC_SCHEMA}.icd10cm_diagnosis_codes_to_ccsr_categories_map")
#ccsr_icd_codesfor
...
# MAGIC Lineage
# MAGIC Insights
# MAGIC concept_code	feature	vocabulary_id
...
fp=ft.unionByName(ft).dropDuplicates()
...
ltables={
    "CCSR":"icd10cm_diagnosis_codes_to_ccsr_categories_map",
}
lcolumns={
    "ICD":"ICD-10-CM_Code",
}
...
for feature_code,ccsrList in featureCCSRDict.items():
    ...
    vlist="|".join(ccsrList)
    catcol=ccolumn[lt]
    sqlstring=f"select distinct LEFT(`{concept_column}`,3)||'.'||RIGHT(rjust(`{concept_column}`,10),2) as concept_code, '{ct}' as vocabulary_id,'{fn}' as feature_name from {SRC_SCHEMA}.{ftable} where `{catcol}` RLIKE '{vlist}'"
    print(sqlstring) 
    values=spark.sql(sqlstring)
    display(values)
    ft_name=f"{{CATALOG}.SRC_SCHEMA}.collections",
    ft=spark.table(ft_name)
    ft.unionByName(values).dropDuplicates()
    fp.write.mode("overwrite").format("delta").saveAsTable(ft_name)
display(fp)
