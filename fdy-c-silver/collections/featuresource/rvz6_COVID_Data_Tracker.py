...
# MAGIC 1. Update the adm_mon to ~2 months earlier than the release (so, July 2021 release would mean we'd display data through May 2021)---*this code is now automated to account for 2 month lag*
# MAGIC               
...
paticd=spark.read.table("edav_prd_cdh.cdh_premier_v2.paticd_diag")
patbill=spark.read.table("edav_prd_cdh.cdh_premier_v2.patbill")
providers=spark.read.table("edav_prd_cdh.cdh_premier_v2.providers")
disstat=spark.read.table("edav_prd_cdh.cdh_premier_v2.disstat")
...
new=(
    patdemo
    .groupBy("medrec_key","pat_key","disc_mon", "disc_mon_seq", "adm_mon", "prov_id", "i_o_ind", "pat_type", "ms_drg", "ms_drg_mdc", "point_of_origin", "adm_type", "disc_status", "mart_status", "age", "gender", "race", "hispanic_ind","admphy_spec", "adm_phy", "attphy_spec", "att_phy", "std_payor", "los", "pat_charges", "pat_cost","pat_fix_cost", "pat_var_cost", "publish_type", "quarter")
    .agg(
...
# MAGIC %md
# MAGIC ``` 
# MAGIC # example to decode from dictionarys.    
# MAGIC from pyspark.sql import functions as F
# MAGIC df = spark.createDataFrame(
# MAGIC     [('A',), ('E',), ('INVALID',)],
# MAGIC     ['col1']
# MAGIC )
# MAGIC dic = {'A': 'S', 'B': 'S', 'E': 'NS'}
# MAGIC
# MAGIC map_col = F.create_map([F.lit(x) for i in dic.items() for x in i])
# MAGIC df = df.withColumn('col2', map_col[F.col('col1')])
# MAGIC
# MAGIC df.show()
# MAGIC # +-------+----+
# MAGIC # |   col1|col2|
...
icd2 = (
    ...
    # Possible values are 'A'dmitting, 'P'rimary, 'S'econdary
...
covid=(
    diagnostic
    .filter(( (F.col("disc_mon")>=2020204) & (F.col("yes_u071")==1) ) | ( (F.col("adm_mon").isin(2020102,2020103,2020204)) & (F.col("disc_mon").isin(2020103,2020204)) & (F.col("yes_b9729")==1)))
    .distinct()
)
...
analysis=(
    ...
    .groupBy("pat_key", "medrec_key", "disc_mon", "disc_mon_seq", "adm_mon", "prov_id", "i_o_ind", "pat_type", "ms_drg", "ms_drg_mdc", "point_of_origin", "adm_type", "disc_status", "mart_status", "age", "gender", "race", "hispanic_ind", "admphy_spec", "adm_phy", "attphy_spec", "att_phy", "std_payor", "los", "pat_charges", "pat_cost", "pat_fix_cost", "pat_var_cost", "publish_type", "n_race_rep", "icd_version", "icd_code", "icd_pri_sec", "icd_poa", "year", "yes_u071", "yes_b9729", "urban_rural", "teaching", "beds_grp", "prov_region", "prov_division", "cost_type", "disc_status_desc")
    .agg(
...
cc_all2=(
    cc_all
    .groupBy("pat_key")
    .agg(F.countDistinct("serv_day").alias("icudays"),
         F.min("serv_day").alias("firsticuday")
        )
    .distinct()
)    
...
cc_all_m2=( 
    cc_all_m
    .groupBy("pat_key")
    .agg(F.countDistinct("serv_day").alias("ventdays"),
         F.min("serv_day").alias("firstventday")
         )
    .distinct()
)    
...
covid_analysis_olddef=(
    ...
    .withColumn("adm_Year",
                F.substring(F.col("adm_mon"),pos=0,len=4),
               )
    .withColumn("adm_Month",
               F.substring(F.col("adm_mon"),pos=6,len=2)
               )
      )
#check this 
...
covid_cohort_final=(
    ...
    .filter( (F.col("adm_mon")>=2020103) & (F.col("adm_date")<= F.col("months2")))
    .orderBy("medrec_key","adm_mon")
    .withColumn("first",F.row_number().over(Window.partitionBy("medrec_key").orderBy(F.col("medrec_key"),F.col("adm_mon"))))
)
...
covid_cohort_final_sdf=(
    ...
    .select('PAT_KEY','MEDREC_KEY','DISC_MON','ADM_MON', 'AGE', 'GENDER','RACE','HISPANIC_IND',  'yes_u071','yes_b9729', 'inpatient','ICU_pat_flag', 'vent_pat_flag', 'inpatient_pat_flag','death_pat_flag', 'raceeth2', 'month','Year','agecat6','agecat4','death','ICU_flag','vent_flag')
)
...
previous_run=spark.read.table('edav_prd_cdh.cdh_premier_exploratory.COVID_DATA_TRACKER').count()
...
.option('header', True)
.option('sep', ',')
.saveAsTable('edav_prd_cdh.cdh_premier_exploratory.COVID_DATA_TRACKER_1')
)
#covid_cohort_final_sdf.write.mode("overwrite").saveAsTable("edav_prd_cdh.cdh_premier_exploratory.COVID_DATA_TRACKER")
