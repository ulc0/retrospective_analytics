...
dbutils.widgets.text('CONCEPT_CODE',defaultValue='`ICD-10-CM_Code`')
...
newcols=['concept_code','feature_name']
src_data = spark.table(src_table).select([CONCEPT_CODE,FEATURE]).toDF(*newcols).withColumn('vocabulary_id',F.lit(VOCABULARY_ID))
