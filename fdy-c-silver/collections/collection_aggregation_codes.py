...
for feature_code,ccsrList in featureCCSRDict.items():
    ...
    vlist="|".join(ccsrList)
    catcol=fcolumn[tbl]
    #sqlstring=f"select distinct {concept_column} as concept_code,'{fn}' as {FEATURE}, '{ct}' as vocabulary_id from {SRC_CATALOG}.{SRC_SCHEMA}.{ftable} where {catcol} RLIKE '{vlist}'"
    sqlstring=f"select distinct {concept_column} as concept_code,'{fn}' as {FEATURE}, '{ct}' as vocabulary_id from {SRC_CATALOG}.{SRC_SCHEMA}.{ftable} where {catcol} RLIKE '{vlist}'"
    print(sqlstring) 
