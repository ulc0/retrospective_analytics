...
def clean_month(col_name='encounterdate'):
    "Return a dataframe where specified column is expressed in YYYY-MM-01 format"
    ...
    def _(input_df):
        ...
        df = (
            ...
            .withColumn(
                "encounter_month",
                F.trunc(F.col("encounterdate").astype("timestamp"), "month")
            )
        )
...
def clean_date_YM(col_name):
    """Return a dataframe with a newly formatted date column in %YMD format appended."""
...
def pp_category_dict(category_dict={
    '1':'ICD',
    'icd9cm':'ICD',
    '9':'ICD',
    '09':'ICD',
    ...
    return  _expand_map(category_dict)
