...
dbutils.widgets.text('input_table_name' , defaultValue="edav_dev_cdh.cdh_ml.note_silver_mimic")
...
renames={"word":"snippet",
         ...
         "score":"score",
         "start":"offset"}
note_nlp=output_df.withColumn('results_exploded',F.explode('results')).select('note_id','results_exploded.*').withColumnsRenamed(renames).withColumn("nlp_system",F.lit(architecture)) #.filter("isNotNull(nlp_category)")
#display(note_nlp)
