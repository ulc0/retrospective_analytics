...
patdemo=spark.read.table("cdh_premier_1cdp_feasibility.patdemo")
if SAMPLE:
    patdemo2 = (
        patdemo
        .select("medrec_key")
    ...
    patdemo.write.mode("overwrite").format("delta").saveAsTable("cdh_premier_ra.twj8_patdemo_sample_01")
    patdemo = spark.table("cdh_premier_ra.twj8_patdemo_sample_01")
    patdemo.count()

paticd=spark.read.table("cdh_premier_1cdp_feasibility.paticd_diag")
if SAMPLE:
    paticd = (
        patdemo
        .select("pat_key")
...
patcpt=spark.read.table("cdh_premier_1cdp_feasibility.patcpt")
if SAMPLE:
    patcpt = (
        patdemo
        .select("pat_key")
...
patlabres=spark.read.table("cdh_premier_1cdp_feasibility.lab_res")
if SAMPLE:
    patlabres = (
        patdemo
        .select("pat_key")
...
genlab=spark.read.table("cdh_premier_1cdp_feasibility.genlab")
if SAMPLE:
    genlab = (
        patdemo
        .select("pat_key")
...
patbill=spark.read.table("cdh_premier_1cdp_feasibility.patbill")
if SAMPLE:
    patbill = (
        patdemo
        .select("pat_key")
...
# Look-up tables
chgmstr=spark.read.table("cdh_premier_1cdp_feasibility.chgmstr")
hospchg = spark.table("cdh_premier_1cdp_feasibility.hospchg")
disstat=spark.read.table("cdh_premier_1cdp_feasibility.disstat")
pattype=spark.read.table("cdh_premier_1cdp_feasibility.pattype")
providers=spark.read.table("cdh_premier_1cdp_feasibility.providers")
zip3=spark.read.table("cdh_premier_1cdp_feasibility.prov_digit_zip")
prov=providers.join(zip3,"prov_id","inner")
payor = spark.table("cdh_premier_1cdp_feasibility.payor")
icd_desc=spark.read.table("cdh_premier_1cdp_feasibility.icdcode")
cptcode = spark.table("cdh_premier_1cdp_feasibility.cptcode")
...
TABLE = "cdh_premier_ra.twj8_pat_covid_tf_f_02"
if WRITE:
    (
        pat_covid_tf
        .write
...
if WRITE:
    TABLE = "cdh_premier_ra.twj8_pat_noncovid_f_03"
...
if WRITE:
    TABLE = "cdh_premier_ra.twj8_pat_cov_noncov_f_04"
...
if WRITE:
    TABLE = "cdh_premier_ra.twj8_pat_cov_noncov_excl_f_04"
...
###############################################################################
if WRITE:
    table = "cdh_premier_ra.twj8_pat_cov_noncov_final_f_05"
...
ccsr1 = (
    ...
    .join(
        ...
        .withColumnRenamed("CCSR_Category","ccsr_category")
...
if WRITE:
    table = "cdh_premier_ra.twj8_pat_ccsr_long_f_06"
    cc3_long.write.mode("overwrite").format("delta").saveAsTable(table)
    cc3_long = spark.table(table)
    cc3_long.count()
    table = "cdh_premier_ra.twj8_pat_ccsr_short_f_06"
...
if WRITE:
    table = "cdh_premier_ra.twj8_pat_var_dict_06"
...
if WRITE:
    table = "cdh_premier_ra.twj8_pat_covariates_f_07"
