...
os.environ["DATABRICKS_HOST"]=base_url_21_dev
...
with open('../jobs/cdh-mlops-scipacy-config-dev.json') as f: 
	data = f.read() 
job_payload = json.loads(data) 
...
response = requests.post(f"{base_url_21_dev}jobs/create", json=job_payload, headers=headers)
if response.ok:
...
for task in new_settings["tasks"]:
  task["job_cluster_key"]=existing_cluster_id
  #print(task)
