...
d1_concept_id=1124957
d2_concept_id=1174888
import plotly.express as px
query=f"""
SELECT
     extract(year from (min(t.drug_era_start_date) over(partition by t.person_id, t.drug_concept_id) )) - p.year_of_birth as age,
     c.concept_name d_concept_name
     FROM drug_era t, person p, concept c
     WHERE t.person_id = p.person_id
       AND t.drug_concept_id = c.concept_id
       AND t.drug_concept_id in ({d1_concept_id},{d2_concept_id})
"""

df = sql(query).groupBy('age','d_concept_name').count().toPandas()
fig = px.histogram(df, x="age", y="count", color="d_concept_name",
...
df = sql("""
            select drug1,drug2, supp_d1d2,  log(supp_d1d2/sqrt(supp_d1*supp_d2)) as lod_cosine_d1d2, log(supp_d1d2)-log(supp_d1) as lod_d1d2
            from COPRESCRIBED order by lod_cosine_d1d2 desc
          """)

pdf=df.toPandas()
pdf_matx=pd.crosstab(pdf.drug1,pdf.drug2,values=pdf.lod_cosine_d1d2,aggfunc='sum')
fig = px.imshow(pdf_matx)
fig.update_layout(
    autosize=False,
    width=700,
    height=700,
